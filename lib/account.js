...
exports.update = function(socket, data) {
  function sendProfile(id, type, account) {
    ...
    request({
      url: account.publicURL + '/ext/' + followType + '/profile',
      qs: {
        type: type,
        account: account
      },
  ...
  function broadcastProfileUpdate(account) {
    ...
    rs.pipe(concat(function(follows) {
      follows.forEach(function(f) {
        sendProfile(f.value.id, 'follow.update', account);
      });
    }));
    ...
    rs.pipe(concat(function(followers) {
      followers.forEach(function(f) {
        sendProfile(f.value.id, 'follower.update', account);
      });
    }));
