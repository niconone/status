...
exports.removeIDFollowing = function(socket, id) {
  ...
  db.get('follow!' + id, function(err, acct) {
    ...
    db.get('profile', function(errAcct, account) {
      if (errAcct) {
        console.log('Error on getting profile: ', err);
        return;
      }
        console.log('got here ' , acct.publicURL + '/ext/unfollow')
      request({
...
exports.removeFollowerIncoming = function(req) {
  ...
  db.del('follower!' + data.account.id, function(err) {
    if (err) {
...
// External check to see if you are still following them
exports.isFollowingIncoming = function(req, reply) {
  let data = req.query;

  db.get('follow!' + data.id, function(err, acct) {
    if (err || !acct) {
      // Not following, send back an unfollow pong
      request({
        url: data.publicURL + '/ext/unfollow',
        qs: {
          type: 'follow.remove',
          account: acct
        },
